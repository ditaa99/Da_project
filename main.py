...
# Plot the frequency of different types of accidents
accident_type_freq.plot(kind='bar', ax=ax)
...
accident_types_over_time = accidentDf.pivot_table(index='AccidentYear', columns='AccidentType_en', aggfunc='size', fill_value=0)
...
accident_types_by_severity = accidentDf.pivot_table(index='AccidentType_en', columns='AccidentSeverityCategory_en', aggfunc='size', fill_value=0)
...
#
# # Plot a heatmap for the correlation between accident types
# plt.pcolor(accident_types_cross_tab, cmap='Blues', edgecolors='k', linewidths=0.5)
#
# # Set the title and labels
...
ax.plot(accidents_per_year.index, accidents_per_year.values, marker='o', linestyle='-', color='b', label='Total Accidents')
...
accidents_per_month_type_2018 = accidents_2018.groupby(['AccidentMonth_en', 'AccidentType_en'], observed=True).size().unstack().reindex(['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'])
...
accidents_per_month_type_2018.plot(kind='bar', stacked=True, ax=ax, color=[colors(i) for i in range(len(accidents_per_month_type_2018.columns))])
